...
# 用不同库读取同一个NIfTI文件
nifti_file = 'sample_data/CT_Philips.nii.gz'
...
dicube.save(dcb_image, 'sample_data/test.dcbs')

# 验证元数据完整性
dicube.save_to_dicom_folder(dcb_image, 'sample_data/roundtrip_test')

# 检查往返转换后的元数据
roundtrip_dcm = pydicom.dcmread('sample_data/roundtrip_test/slice_0000.dcm')
...
```{python}
# 比较压缩效果
dcb_size = os.path.getsize('sample_data/test.dcbs')
...
os.remove('sample_data/test.dcbs')
```
...
# DiCube加载性能
start_time = time.time()
dcb_loaded = dicube.load('sample_data/test.dcbs')
