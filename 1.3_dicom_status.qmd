...
一个处于`CONSISTENT`状态的DICOM序列应该具备：

1. **统一的像素间距**：所有切片的PixelSpacing相同
2. **一致的图像尺寸**：所有切片的Rows和Columns相同  
3. **规律的切片间距**：Z轴方向上切片位置呈等间距分布
4. **统一的图像方向**：所有切片的ImageOrientationPatient相同
5. **连续的实例编号**：InstanceNumber从1开始连续递增
6. **完整的元数据**：所有必需的DICOM标签都存在且有效
