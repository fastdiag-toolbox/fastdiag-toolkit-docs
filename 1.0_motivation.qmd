...
1.  统一文件容器：将序列整合为单一文件，根除碎片化。
2.  智能元数据：分离共享/非共享元数据，建立高效索引。
3.  现代编解码：采用 HTJ2K 等技术，优化压缩率与编解码速度。
4.  100% 往返兼容：与标准 DICOM 生态无缝衔接。
...
1.  无损转换：从 DICOM 到 DiCube，再回到 DICOM，像素与元数据无信息损失；
2.  元数据完整：保留所有标准及私有 DICOM 标签；
3.  工作流兼容：可随时将 `.dcbs` 导出为标准 DICOM 文件集，用于 PACS 归档或与存量系统交互。
...
1.  即时性能收益：无需改动上层业务，即可获得显著 I/O 与处理性能提升；
2.  赋能未来应用：为 AI 训练、实时分析、大规模并发等场景打好数据底座；
3.  零风险迁移：完整往返兼容，便于与现有 DICOM 生态无缝集成与回退。
