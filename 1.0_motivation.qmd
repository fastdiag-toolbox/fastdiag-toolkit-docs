...
dirname = 'sample_data/sample_200_0'
...
def concurrent_read_test(num_processes=5):
    ...
    for i in range(num_processes):
        test_dir = f"{dirname[:-1]}{i}"  # sample_200_0 到 sample_200_4
...
dicube.save(dcb_image, 'sample_data/test_sequence.dcbs')

# 测试DiCube加载性能
start_time = time.time()
loaded_image = dicube.load('sample_data/test_sequence.dcbs')
...
# 快速加载元数据（不含像素数据）
start_time = time.time()
meta = dicube.load_meta('sample_data/test_sequence.dcbs')
...
dcb_size = os.path.getsize('sample_data/test_sequence.dcbs')
...
# 清理测试文件
os.remove('sample_data/test_sequence.dcbs')
```
...
dcb_concurrent_time = dcb_concurrent_test('sample_data/test_sequence.dcbs')
...
dicube.save(original_dcb, 'sample_data/roundtrip_test.dcbs')

# 转换回DICOM
loaded_dcb = dicube.load('sample_data/roundtrip_test.dcbs')
dicube.save_to_dicom_folder(loaded_dcb, 'sample_data/roundtrip_dicom')

# 验证元数据完整性
original_dcm = pydicom.dcmread(os.path.join(dirname, os.listdir(dirname)[0]))
roundtrip_dcm = pydicom.dcmread('sample_data/roundtrip_dicom/slice_0000.dcm')
...
# 清理测试文件
os.remove('sample_data/roundtrip_test.dcbs')
```
